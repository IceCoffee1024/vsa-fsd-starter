...
本仓库用于讲解和演示垂直切片架构（Vertical Slice Architecture，VSA）与 Feature-Sliced Design（FSD）如何在全栈系统中协同使用。仓库将可复用指南、面向特定技术栈的最小启动模板以及面向生产实践的示例分开，避免教学用业务代码混入纯净模板。

## 仓库边界

- `docs/` 是所有技术栈共享规则的权威来源。
- `templates/` 下每个已完成的目录都是独立启动模板，并且必须记录自己的运行环境和命令。
- `examples/` 下每个已完成的目录都是基于共享原则组装、可独立运行的全栈应用。
- 模板和示例可以引用文档，但不能依赖其他模板或示例中的运行时代码。
- 示例可以复制并调整模板代码，但模板不能依赖示例。
...
| `templates/frontend-*` | 在单个前端技术栈中演示 FSD 依赖方向和公共 API。 |
| `examples/legacy-ordering/` | 演示端到端现代化改造中的关注点，同时避免把模板变成示例产品。 |
| `scripts/` | 在存在可运行目标后承载仓库级自动化。 |
...
其他模板和全栈示例仍是脚手架。未来每个实现都将负责自己的运行说明；全栈示例还将记录组合后的启动和部署流程。

## 质量属性

- **局部性：** 一项业务变更应主要影响一个后端切片和范围最小的相关前端切片。
- **可替换性：** 面向不同技术栈的模板保持相互独立。
- **可追踪性：** 示例应将其实现选择链接回共享规则。
