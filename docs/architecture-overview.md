...
The Vue 3 template provides an independently runnable FSD frontend with complete Orders CRUD, Customer creation and lookup by identifier, Basic and OAuth authentication interfaces, protected routing, and refresh-token rotation. Its Customer surface follows the currently published backend contract and does not invent unsupported list, update, or delete operations. Other templates remain scaffolds. Each future implementation will own its runtime instructions and verification flow.
