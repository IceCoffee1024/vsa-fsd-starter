...
The `Authorization` header takes precedence when both forms are present, including when the header contains an invalid bearer token; the query token is not used as a fallback. Query-string tokens can leak through logs, browser history, telemetry, and `Referer` headers. Prefer the bearer header; if this compatibility path is required, use HTTPS and redact `access_token`.

### Production security boundary

This local self-hosted demonstration deliberately permits a shared credential in `App.config`, the password grant, plaintext HTTP by default, self-contained access tokens, and query-string token compatibility. These choices do not form a production identity system, and a public `client_id` identifies a client without authenticating it.

A production deployment requires HTTPS, protected secret storage and rotation, stable shared data-protection keys, real user and client stores, and a flow appropriate to the client and threat model, such as authorization code with PKCE for public clients or a confidential-client flow. It also requires administrative revocation and expired-token cleanup. Refresh-token replay revokes future refreshes in that token family, but already-issued self-contained access tokens remain valid until their one-hour expiry.
...
- `Microsoft.Owin.Security` supplies the generic Katana authentication abstractions used by the project's custom Basic options, middleware, handler, tickets, and challenge lifecycle. `Microsoft.Owin.Security.OAuth` supplies the OAuth authorization-server and bearer implementation, including its refresh-token provider integration. `SchemeAwareAuthenticationFilter` selects the OWIN challenge for the incoming scheme; Web API's global `AuthorizeAttribute` separately enforces authentication.
